...
def SRR(circle_radius=7.3 * 1e-3, gap=1.5 * 1e-3, delta_phi=-np.pi / 2, wire_radius=0.5 * 1e-3, num_of_wires=12):
...
def double_srr_6GHz(r=3.25 * 1e-3, p0=(0, 0, 0), wr=0.25 * 1e-3, num=12):
    g = double_SRR(inner_radius=r - 1e-3, outer_radius=r, wire_radius=wr, num_of_wires=num)
    g.translate(p0)
